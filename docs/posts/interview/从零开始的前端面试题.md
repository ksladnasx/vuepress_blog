...
src 是 _source_ 的缩写，指向外部资源的位置，指向的内容将会嵌入到文档中当前标签所在位置；在请求 src 资源时会将其指向的资源下载并应用到文档内，例如 js 脚本，img 图片和 frame 等元素。
...
href 是 _Hypertext Reference_ 的缩写，指向网络资源所在位置，建立和当前元素（锚点）或当前文档（链接）之间的链接，如果在文档中添加
...
> 注意：Vue 本身的 script 标签默认没有 defer 或 async 属性，如果是 CDN 方式引入 Vue，普通 script 会同步加载并阻塞 HTML 解析，实际项目中通常会加 defer 来避免阻塞。对于 Vue CLI 或 Vite 项目，由于使用 ES Module，`type="module"` 本身具有类似 defer 的效果，会异步加载并在 HTML 解析完成后执行。async 一般不用于 Vue，因为它无法保证脚本执行顺序，容易出现 Vue 未加载完成业务代码已经执行的问题。
...
- HTML5 不再依赖 DTD 来定义文档类型，其 `<!DOCTYPE html>` 主要用于告诉浏览器采用标准模式。虽然 HTML5 本身没有基于 DTD 的严格版和过渡版之分，但浏览器为了兼容历史网页，仍然保留标准模式和混杂模式两种渲染机制。
- HTML5 还需要 `<!DOCTYPE html>`不是为了引用 DTD，而是为了让浏览器进入标准模式渲染页面，避免进入混杂模式导致不同浏览器出现兼容性问题

总之，**严格模式让各个浏览器统一执行一套规范兼容模式保证了旧网站的正常运行。**

- HTML 严格模式（Strict DTD）—— HTML4 的概念

- 浏览器标准模式（Standards Mode）—— 浏览器渲染模式

- JavaScript Strict Mode（`"use strict"`）—— JS 语法约束
...
- 即使容器有剩余空间，项目也不会变大。项目保持原始尺寸

### 2. `flex-shrink: 1`（会收缩）

- 当容器空间不足时，项目会缩小以适应容器。这是为了避免内容溢出

### 3. `flex-basis: auto`（基准值为内容大小）

- 项目的基准尺寸取元素本来的宽度（`width` 或内容宽度）。如果设置了 `width` 就按 `width`，没设置就按内容宽度

flex属性默认值为0 1 auto。 **flex:1 表示 flex: 1 1** **0%**：

- 第一个参数表示: **flex-grow 定义项目的放大比例，此处为1，即如果存在剩余空间，就进行放大；**

其他常用flex简写：

| 写法            | 等价于     | 含义                                   |
| :-------------- | :--------- | :------------------------------------- |
| `flex: initial` | `0 1 auto` | **默认值**：可收缩，不可放大           |
| `flex: 0`       | `0 1 0%`   | 不放大，基准为 0（均分宽度）           |
| `flex: 1`       | `1 1 0%`   | 可放大可收缩，基准为 0（等分剩余空间） |
| `flex: auto`    | `1 1 auto` | 可放大可收缩，基准为内容大小           |
| `flex: none`    | `0 0 auto` | 固定尺寸，不放大不收缩                 |

- **默认情况**：什么都不写，就是 `0 1 auto`
- **想让项目等分宽度**：写 `flex: 1`
- **想让项目固定不动**：写 `flex: none`
- **想让项目按内容比例分配**：写 `flex: auto`
...
```html
ul { padding: 0; } li { list-style: none; text-align: center; border-radius:
...
![img](..\img\从零开始的前端面试题\1615475711487-c474af95-b5e0-4778-a90b-9484208d724d.png)
...
![img](..\img\从零开始的前端面试题\1605247722640-5bcb9156-a8b4-4d7c-83d7-9ff80930e1de.jpeg)
...
![img](..\img\从零开始的前端面试题\1604021075237-8249a8df-3a28-4bca-9f22-02923aba8618.png)
...
![img](..\img\从零开始的前端面试题\1605099411873-d2eac25a-5d8c-4586-bc36-769bce79010e.png)
