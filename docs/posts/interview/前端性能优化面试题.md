...
![img](..\img\前端性能优化面试题\1603966294889-153271b5-4b9f-4470-b05f-c7a9f030d043.png)
...
![img](..\img\前端性能优化面试题\1603966605254-fe880ec0-ebd1-4f94-b662-cdd5e5396c34.png)
...
<body>
    <div class="container">
        <img src="loading.png" alt="" data-src="img1.png">
        <img src="loading.png" alt="" data-src="img1.png">
        <img src="loading.png" alt="" data-src="img1.png">
        <img src="loading.png" alt="" data-src="img1.png">
        <img src="loading.png" alt="" data-src="img1.png">
...
<script>
    ...
    function lozyLoad() {
        ...
        for (let i = 0; i < imgs.length; i++) {
            let img = imgs[i];
            // 关键修正：只处理还没有真实 src 的图片（即仍然有 data-src 属性）
            if (img.getAttribute('data-src') && img.offsetTop < scrollTop + winHeight) {
                img.src = img.getAttribute('data-src');
                img.removeAttribute('data-src');  // 加载后移除 data-src属性，避免重复加载
                console.log('加载图片', i);
            }
        }
    }

    // 将 lozyLoad 这个函数本身（而不是它的执行结果）赋值给 window 对象的 onscroll 事件处理属性。
    //这样，当页面发生滚动时，浏览器会自动调用 lozyLoad 函数。
    window.onscroll = lozyLoad; //或者用 window.addEventListener('scroll', lozyLoad);
    // 页面加载时先执行一次，加载首屏图片
    lozyLoad();
</script>
<style>
