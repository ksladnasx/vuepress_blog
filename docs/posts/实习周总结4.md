...
主要这里要多个v-model才能双向数据流动，直接冒号是单向传递的。

#### 父组件：

```vue
<template>
  <index v-model:name="userName" v-model:email="userEmail" v-model:age="age" />
...
const userName = ref('初始名称');
const userEmail = ref('初始邮箱');
const age = ref(0);
</script>
```
...
<template>
  ...
  <el-form ref="formRef" :model="form" label-width="auto"> 
      ...
      <el-form-item label="Age" prop="newAge">
        <el-input v-model="form.newAge" placeholder="请输入" />
      </el-form-item>
  </el-form>
    <el-button type="primary" @click="updateName(form.newName);updateEmail(form.newEmail);updateAge(form.newAge)">提交</el-button>
</template>
