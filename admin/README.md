...
令牌会保存到当前浏览器的 localStorage 中，刷新页面后会自动尝试进入后台；点击“退出后台”会清除已保存的令牌。后台还会使用浏览器 IndexedDB 保存未发布的文章草稿、待删除状态和待上传图片；这些内容只保存在当前浏览器里。编辑内容会标记为“未保存”，可手动保存；切换文章前会自动保存，连续三十秒没有继续编辑时也会兜底自动保存。新文章必须选择存放目录并填写一级标题后才能保存或发布。
